...
bloque1.addEventListener('click', () => {
  esCorrecta(bloque1.id), botonDesaparece(), borderColor(bloque1);
});
bloque2.addEventListener('click', () => {
  esCorrecta(bloque2.id), botonDesaparece(), borderColor(bloque2);
});
bloque3.addEventListener('click', () => {
  esCorrecta(bloque3.id), botonDesaparece(), borderColor(bloque3);
});
bloque4.addEventListener('click', () => {
  esCorrecta(bloque4.id), botonDesaparece(), borderColor(bloque4);
});
...
const limite = async () => {
  if (preguntasCompletadas === 10) {
    let setFirebase = await addDoc(
      collection(db, 'halloween_quiz', auth.currentUser.uid, 'attempts'),
      {
